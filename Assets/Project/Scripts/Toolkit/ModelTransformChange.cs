...
public sealed class ModelTransformChange : MonoBehaviour
{
    ...
    private void ModelRotation()
    {
        ...
        if (Input.touchCount == 1)
        {
            ...
            {
                float deltaX = touch.deltaPosition.x; // 水平滑动量
                //float deltaY = touch.deltaPosition.y; // 垂直滑动量

                // 左右旋转（绕 Y 轴）
                NeedChangeObject.transform.Rotate(0, -deltaX * rotationSpeed, 0, Space.Self);
                // 上下旋转（绕 X 轴）
                //NeedChangeObject.transform.Rotate(-deltaY * rotationSpeed, 0, 0, Space.Self);
            }
        }
    }
